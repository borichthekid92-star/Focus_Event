...
def api_rules():
    ...
    # Use detection from request if it exists, otherwise build from pattern field
    if 'detection' in data and isinstance(data['detection'], dict):
        detection = data['detection']
    else:
        # Simplified detection - just a single pattern
        detection = {
    ...
    rule = {
        ...
        'detection': detection,
        'meta': data.get('meta', {'enabled': True, 'hit_count': 0})
    }

    save_rule(rule, user_id)
    return jsonify(rule), 201

@app.route("/api/alerts", methods=["GET"])
@login_required
def api_alerts():
    """Fetch all triggered rules (rules with hit_count > 0) from Rules.db"""
    try:
        conn = sqlite3.connect(RULES_DB)
        ...
        # Fetch all rules
        cursor.execute("SELECT * FROM rules ORDER BY updated_at DESC")
        rows = cursor.fetchall()
        conn.close()
        ...
        for row in rows:
            try:
                ...
                meta = json.loads(row['meta']) if row['meta'] else {'enabled': True, 'hit_count': 0}
                tags = json.loads(row['tags']) if row['tags'] else []
                
                # Only include rules with hits (triggered rules)
                ...
                logger.warning(f"Error parsing rule {row['id']}: {e}")
                continue
        
        # Filter to only triggered rules
        triggered_alerts = [a for a in alerts if a['hit_count'] > 0]
...
def matches_rule(event, rule):
    """Check if an event matches a rule's detection pattern"""
    try:
        detection = rule.get('detection', {})
        pattern = detection.get('pattern', '.*')
        
        # Get regex flags (case-insensitive by default)
        flags = re.IGNORECASE
        
        # Concatenate all event fields into one searchable string
        event_text = ' '.join(str(v) for k, v in event.items() if v)
        
        # Check if pattern matches anywhere in the event text
        try:
            if pattern and pattern != '.*':
                if not re.search(pattern, event_text, flags):
                    return False
...
@app.route("/api/rules/match-events", methods=["POST"])
@login_required
def match_events_to_rules():
    """Match events against all rules and return matched rules"""
    try:
        data = request.get_json() or {}
        ...
        cursor.execute("""
            SELECT * FROM transferred_events 
        ...
        if not events:
            return jsonify({'success': True, 'matched_rules': [], 'matched_events': 0})
        ...
        rules_cursor.execute("""
            SELECT * FROM rules 
            WHERE json_extract(meta, '$.enabled') = 1 OR json_extract(meta, '$.enabled') = true
        """)
        ...
        # Check each event against each rule
        for event in events:
            for rule in rules:
                try:
                    rule_dict = {
                        'detection': json.loads(rule['detection']) if rule['detection'] else {},
                        'meta': json.loads(rule['meta']) if rule['meta'] else {}
                    }
                    
                    if matches_rule(event, rule_dict['detection']):
                        matched_count += 1
                        ...
                        triggered_rules[rule_id]['match_count'] += 1
                        
                        # Trigger the rule (increment hit count)
                        update_rule_hit_count(rule_id)
        ...
        logger.error(f"Error in match_events_to_rules: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

def update_rule_hit_count(rule_id):
    """Update hit count for a triggered rule"""
    try:
        conn = sqlite3.connect(RULES_DB)
        cursor = conn.cursor()
        
        cursor.execute("SELECT meta FROM rules WHERE id = ?", (rule_id,))
        row = cursor.fetchone()
        
        if row:
            meta = json.loads(row[0]) if row[0] else {'enabled': True, 'hit_count': 0}
            meta['hit_count'] = meta.get('hit_count', 0) + 1
            meta['last_triggered'] = datetime.now().isoformat()
            
            cursor.execute(
                "UPDATE rules SET meta = ?, updated_at = ? WHERE id = ?",
                (json.dumps(meta), datetime.now().isoformat(), rule_id)
            )
            conn.commit()
        
        conn.close()
    except Exception as e:
        logger.error(f"Error updating rule hit count: {e}")

@app.route("/api/rules/<rule_id>/trigger", methods=["POST"])
@login_required
def trigger_rule(rule_id):
    """Increment hit count for a rule when it's triggered"""
    try:
        conn = sqlite3.connect(RULES_DB)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        # Update meta with incremented hit_count
        meta = json.loads(row['meta']) if row['meta'] else {'enabled': True, 'hit_count': 0}
        meta['hit_count'] = meta.get('hit_count', 0) + 1
        meta['last_triggered'] = datetime.now().isoformat()
        ...
        return jsonify({'success': True, 'hit_count': meta['hit_count']})
    except Exception as e:
...
@app.route("/api/rules/all", methods=["GET"])
@login_required
def get_all_rules():
    """Get all rules from the rules database"""
    try:
        conn = sqlite3.connect(RULES_DB)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        cursor.execute("""
            SELECT id, user_id, name, severity, tags, description, 
                   version, created_at, updated_at, detection, meta 
            FROM rules 
            ORDER BY 
                CASE severity 
        ...
        for row in rows:
            ...
            rules.append({
                ...
                'enabled': meta.get('enabled', True),
                'hit_count': meta.get('hit_count', 0),
                'last_triggered': meta.get('last_triggered', None),
                'created_at': row['created_at'],
                'updated_at': row['updated_at'],
                'version': row['version']
            })
